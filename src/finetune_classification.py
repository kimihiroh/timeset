"""
Finetune sequence classification model
for TEMPORAL-NLI
...
from my_datasets import FinetuneDatasetForClassificationModel
...
def validate_model(
    ...
    """
    validate model by calculating loss and f1 of 1
    """
    ...
    model.eval()
    with torch.no_grad():
        for batch, labels_batch in tqdm(dataloader):
            outputs = model(
    ...
    logging.debug(f"pred: {preds[idx]}")
    logging.debug(f"gold: {labels[idx]}")

    report = classification_report(
        labels,
        preds,
        labels=[x for x in range(len(categories))],
    ...
    accuracy = sum(np.array(labels) == np.array(preds)) / len(labels)
    macro_f1 = report["macro avg"]["f1-score"]

    logging.info(f"Loss (dev): {avg_loss:.4f}")
    logging.info(f"Accuracy (dev): {accuracy:.4f}")
    logging.info(f"Macro F1 (dev): {macro_f1:.4f}")
...
def train_one_epoch(
    ...
    model.train()
    for idx, (input_encodings_batch, labels_batch) in enumerate(tqdm(dataloader)):
...
def main(args):
    ...
    dataset_train = FinetuneDatasetForClassificationModel(
        dataset_name=args.dataset_name,
        filepath_data=args.filepath_data_train,
        tokenizer=tokenizer,
    )
    dataset_dev = FinetuneDatasetForClassificationModel(
        dataset_name=args.dataset_name,
    ...
    _dirpath_model_weight = (
        ...
        f"_{args.finetune_type}_sequence_classification"
    )
    ...
    for epoch in range(args.num_epoch):
        ...
        loss_dev, accuracy, macro_f1, report = validate_model(
        ...
        if best_score < accuracy:
            best_score, best_epoch = accuracy, epoch
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Finetuning for sequence classification")
